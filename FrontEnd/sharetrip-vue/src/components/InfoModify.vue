...
const modify = () => {
	...
	axios
		.put(`/user/modify/${user.value.id}`, user.value)
		.then(() => {
			...
			<v-container>
				<v-form v-model="form">
					...
					<v-radio-group v-model="user.gender" inline label="성별을 선택하세요">
						<v-radio label="Female" color="indigo-darken-3" value="여성"></v-radio>
						<v-radio label="male" color="indigo-darken-3" value="남성"></v-radio>
						<v-radio label="private" color="indigo-darken-3" value="비공개"></v-radio>
					</v-radio-group>
				</v-form>
			</v-container>
